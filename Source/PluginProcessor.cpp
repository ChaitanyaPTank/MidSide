...
#endif
{
	addParameter(mSideGain = new juce::AudioParameterFloat("mSideGain", "side gain", 0.0f, 1.0f, 1.0f));
	addParameter(mMidGain = new juce::AudioParameterFloat("mMidGain", "mid gain", 0.0f, 1.0f, 1.0f));
}
...
void MidSideAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
	...
	for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
	{
		...
		float mid = (inputL + inputR) * *mMidGain;
		float side = (inputL - inputR) * *mSideGain;
